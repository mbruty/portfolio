  ...
  renderContent() {
    switch (this.props.element) {
      case "Sorting Algorithm Visualiser":
        return (
          <div id="sorting-vis">
            <SortingVis width={window.innerWidth} height={window.innerHeight} />
          </div>
        );
      case "URL Shortener":
        return (
      ...
      case "Starwars Guild Inspector (Example Allycodes in the read me)":
        return (
          <div
            className="hue-says-container"
            style={{
  ...
  render() {
    return (
      <div className="chrome-window">
        <div className="nav">
          <i className="material-icons noselect" onClick={this.props.close}>
            arrow_back_ios
          </i>
          <h3>{this.props.element}</h3>
        </div>
        <div className="chrome-content">{this.renderContent()}</div>
      </div>
    );
  }
}
