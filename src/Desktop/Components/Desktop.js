...
export default class Desktop extends Component {
  ...
  showWindow(e) {
    ...
    const iconClass = e.target.getElementsByTagName("i")[0].className;
    ...
    if (window) {
      this.bringWindowToFront({
        id: window.windowToShow,
        z: window.z,
        bypass: true,
      });
    } else {
      openWindowsCoppy.push({
        showChrome: true,
        windowToShow: target,
        z: top,
        icon: icon,
  ...
  unMinimize(e) {
    ...
    const z = this.state.openWindows.filter(
      (x) => x.windowToShow === target
    )[0];
    ...
    return (
      <>
        <div className="desktop noselect">
          {this.state.openWindows.map((element) => (
            <ChromeWindow
              minimize={this.minimize}
              show={element.showChrome}
              target={element.windowToShow}
              closeChrome={this.closeChrome}
              bringToFront={this.bringWindowToFront}
              z={element.z}
        ...
        <StartBar
          ...
          unMinimize={this.unMinimize}
          showWindow={this.showWindowFromString}
        />
      </>
    );
  }
